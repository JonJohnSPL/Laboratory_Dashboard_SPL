...
function renderResources(){
  ...
  const donesafeEquipment = renderResourceCards(state.data.donesafeEquipment.filter((item) => item.isActive !== false), (item) => `<div class="resource-card"><div class="resource-card-head"><div><div class="item-title">${esc(item.assetName || item.recordTitle || `DoneSafe record ${item.donesafeRecordId}`)}</div><div class="muted">${esc(item.assetType || 'Equipment Tracker')}</div></div><div class="mini-tags"><span class="tag-chip">DoneSafe</span>${item.assetStatus ? getStatusBadge(item.assetStatus) : ''}</div></div><div class="mini-tags">${item.serialNumber ? `<span class="tag-chip">Serial: ${esc(item.serialNumber)}</span>` : ''}${item.inventoryBarcode ? `<span class="tag-chip">Asset: ${esc(item.inventoryBarcode)}</span>` : ''}</div><div class="muted">${esc([item.manufacturer, item.model].filter(Boolean).join(' | ') || 'No manufacturer or model')}</div><div class="muted">${esc(item.assetLocation || 'No location supplied')}</div>${item.nextInspectionDue ? `<div class="muted">Inspection due: ${esc(fmtDate(item.nextInspectionDue))}</div>` : ''}${item.sourceUrl ? `<div><a class="inline-link" href="${esc(item.sourceUrl)}" target="_blank" rel="noopener">Open in DoneSafe</a></div>` : ''}</div>`, 'No synchronized DoneSafe equipment');
