...
let saveTimer = null;
const storageAdapter = ( window.storage && typeof window.storage.get === 'function' && typeof window.storage.set === 'function' ) ? window.storage : { get: async (key) => ({ value: localStorage.getItem(key) }), set: async (key, value) => { localStorage.setItem(key, value); } };
function showSaveStatus(state, msg) { const el = document.getElementById('save-indicator'); el.style.visibility = 'visible'; el.className = 'save-indicator ' + state; el.textContent = msg; } async function saveData() { showSaveStatus('saving', ' SAVING...'); try { await storageAdapter.set(STORAGE_KEY, JSON.stringify(WOs)); showSaveStatus('saved', ' SAVED'); } catch (e) { showSaveStatus('error', ' SAVE FAILED'); console.error('Storage save error:', e); } }
function scheduleSave() { clearTimeout(saveTimer); saveTimer = setTimeout(saveData, 600); } async function loadData() { try { const result = await storageAdapter.get(STORAGE_KEY); if (result && result.value) { const parsed = JSON.parse(result.value); if (Array.isArray(parsed)) { WOs = parsed; render(); showSaveStatus('loaded', ` ${WOs.length} WOs LOADED`); setTimeout(() => { document.getElementById('save-indicator').style.visibility = 'hidden'; }, 3000);
return true; } } } catch (e) { console.log('No saved data found, loading demo.'); } return false;
} saveData = async function() { showSaveStatus('saving', 'SAVING...'); try { normalizeScheduleState(); await Promise.all([ storageAdapter.set(STORAGE_KEY, JSON.stringify(WOs)), storageAdapter.set(SCHEDULE_STORAGE_KEY, JSON.stringify(scheduleState)) ]); showSaveStatus('saved', 'SAVED'); } catch (e) { showSaveStatus('error', 'SAVE FAILED'); console.error('Storage save error:', e); }
}; loadData = async function() { let loadedWOs = false;
let loadedSchedule = false; try { const [woResult, scheduleResult] = await Promise.all([ storageAdapter.get(STORAGE_KEY), storageAdapter.get(SCHEDULE_STORAGE_KEY) ]); if (woResult && woResult.value) { const parsed = JSON.parse(woResult.value); if (Array.isArray(parsed)) { WOs = normalizeWorkOrders(parsed); loadedWOs = true; } else if(parsed && Array.isArray(parsed.workOrders)) { WOs = normalizeWorkOrders(parsed.workOrders); loadedWOs = true; } } if (scheduleResult && scheduleResult.value) { const parsedSchedule = JSON.parse(scheduleResult.value);
if(parsedSchedule && typeof parsedSchedule === 'object') { scheduleState = { date: parsedSchedule.date || todayISO(), employees: Array.isArray(parsedSchedule.employees) ? parsedSchedule.employees : [], entries: Array.isArray(parsedSchedule.entries) ? parsedSchedule.entries : [], tasks: Array.isArray(parsedSchedule.tasks) ? parsedSchedule.tasks : [] }; loadedSchedule = true; } } normalizeScheduleState(); if (loadedWOs || loadedSchedule) { showSaveStatus('loaded', `${WOs.length} WOs | ${scheduleState.entries.length} scheduled`); setTimeout(() => { document.getElementById('save-indicator').style.visibility = 'hidden'; }, 3000); } } catch (e) { console.log('No saved data found.'); } return loadedWOs || loadedSchedule;
};
...
(async function init() { initColumnVisibility(); renderColumnSelector(); await (window.authReadyPromise || Promise.resolve()); await loadData(); render(); renderSchedule(); switchView('queue');
})();
