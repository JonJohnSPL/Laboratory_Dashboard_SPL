...
function normalizeTestDefinition(def, index = 0){ const key = normalizeCatalogKey(def?.key || def?.testCode || def?.test_code || def?.code || def?.label || def?.displayLabel || def?.display_label || `TEST_${index + 1}`); if(!key) return null; const label = String(def?.label || def?.displayLabel || def?.display_label || key).trim() || key; const aliasSource = Array.isArray(def?.aliases) ? def.aliases : String(def?.aliases || '').split(','); const aliases = uniqueList([key, label, ...aliasSource]); const matrixType = inferMatrixType(def); const groupKey = normalizeCatalogKey(def?.groupKey || def?.group_key || ''); return { id:String(def?.id || key), key, label, shortLabel:String(def?.shortLabel || def?.short_label || label).trim() || label, minutes:normalizeMinutesForMatrixType(matrixType, def?.minutes), countMode:(def?.countMode || def?.count_mode) === 'perRow' ? 'perRow' : 'perSample', matrixType, groupKey, groupRank:Math.max(0, Number(def?.groupRank ?? def?.group_rank ?? 0)), aliases, tone:matrixType === 'Liquid' ? 'liq' : inferTestTone({ key, groupKey }), sortOrder:Number.isFinite(Number(def?.sortOrder ?? def?.sort_order)) ? Number(def.sortOrder ?? def.sort_order) : index, columnWidth:getColumnWidthForTest({ label }) }; }
...
function openTestCatalogModal(){ if(isRemoteStorageMode()){ window.location.href='test-types.html'; return; } resetTestCatalogForm(); renderTestCatalogList(); document.getElementById('test-catalog-overlay').classList.add('open'); }
...
const testDefinitionsPromise = isRemoteStorageMode() ? window.appAuth.requestJson('/rest/v1/lab_test_types?select=*&is_active=eq.true&lab_wip_enabled=eq.true&order=sort_order.asc,test_code.asc') : storageAdapter.get(TEST_DEFINITION_STORAGE_KEY);
const [woResult, scheduleResult, testDefinitionsResult, subcontractLabsResult, sharedDirectoryChanged] = await Promise.all([ storageAdapter.get(STORAGE_KEY), storageAdapter.get(SCHEDULE_STORAGE_KEY), testDefinitionsPromise, storageAdapter.get(SUBCONTRACT_LABS_STORAGE_KEY), loadSharedDirectory() ]);
...
const testDefinitionsRaw = isRemoteStorageMode() ? JSON.stringify(Array.isArray(testDefinitionsResult) ? testDefinitionsResult : []) : (typeof testDefinitionsResult?.value === 'string' ? testDefinitionsResult.value : '');
